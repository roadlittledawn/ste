...
def get_orgs_ids
    $orgs_ids_arr = get_data('http://localhost:9292/orgs')
    return $orgs_ids_arr
end

def get_org_obj (org_id)
    orgs_arr_item = get_data("http://localhost:9292/orgs/#{org_id}")
...
def get_accounts_ids
    $accts_ids_arr = get_data('http://localhost:9292/accounts')
    return $accts_ids_arr
end

def get_acct_obj (acct_id)
    acct_arr_item = get_data("http://localhost:9292/accounts/#{acct_id}")
...
def get_all_user_ids
    $users_ids_arr = get_data('http://localhost:9292/users')
...
def get_user_obj (user_id)
    user_obj = get_data("http://localhost:9292/users/#{user_id}")
    return user_obj
end

def get_user_ids_by_org (org_id)
    user_obj = get_data("http://localhost:9292/users/org/#{org_id}")
    return user_obj
end

def get_admin_user_ids_by_org (org_id)
    user_obj = get_data("http://localhost:9292/users/org/#{org_id}/admin")
    return user_obj
end
...
def get_org_users_obj_by_role (org_id, role)
    if role == 'admin'
        user_ids = get_data("http://localhost:9292/users/org/#{org_id}/admin")
        # separate loop?
